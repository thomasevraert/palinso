    ...
    <tr>
      ...
      <td class="category-cell" data-id="${a.id}">
        ...
        <div class="category-edit-form">
          <input class="category-input" type="text" value="${category}"
            placeholder="Catégorie..." list="category-suggestions" />
          <datalist id="category-suggestions">
            ${[...new Set(allArticles.map(x => x.category).filter(Boolean))]
...
function loadSubscription() {
  ...
  function updatePrices(isAnnual) {
    ...
    document.querySelectorAll('.plan-price').forEach(function(el) {
      ...
      el.textContent = parseFloat(val).toFixed(2).replace('.', ',');
    });
  ...
  freshTrialCta.addEventListener('click', function() {
  ...
  document.querySelectorAll('.plan-btn').forEach(function(btn) {
    ...
    freshBtn.addEventListener('click', function() {
      ...
      chrome.storage.local.set({
        subscription: { plan: plan, billing: billing, since: new Date().toISOString() }
      }, loadSubscription);
    });
  });
